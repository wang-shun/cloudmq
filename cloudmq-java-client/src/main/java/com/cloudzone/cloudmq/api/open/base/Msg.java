...
public class Msg extends com.alibaba.rocketmq.common.message.Message implements Serializable {
    ...
    /*
     * public long getStartDeliverTime() { String pro =
     * this.getSystemProperties("__STARTDELIVERTIME"); return pro !=
     * null?Long.parseLong(pro):0L; }
     * 
    ...
    public void setDelayTimeLevel(int level) {
        ...
        if (level < 0 || level > DelayLevelConst.TwoHour.val()) {
            String errMsg = String.format("[ERROR]: Current Level == %d 消息延迟Level值设置错误，请重新设置!", level);
    ...
    public String toString() {
        String format = "Msg [topic=%s, systemProperties=%s, userProperties=%s, body.length=%d, msgId=%s]";
        Integer bodyLength = this.body != null ? this.body.length : 0;
        return String.format(format, this.topic, this.systemProperties, this.userProperties, bodyLength,
            this.getMsgId());
    }
