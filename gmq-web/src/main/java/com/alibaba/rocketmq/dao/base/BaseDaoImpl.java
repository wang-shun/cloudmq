...
public abstract class BaseDaoImpl<T, KEY extends Serializable> extends MyBatisSupport implements BaseDao<T, KEY> {
    ...
    private static final String DEFAULT_INSERT_KEY_MAP = "insertEntryMap";
    ...
    public Integer insertEntryMap(Map<String, Object> p) {
        Integer result = this.insertByMap(getNameSpace(DEFAULT_INSERT_KEY_MAP), p);
        return result != null ? result : 0;
    }


    @SuppressWarnings("unchecked")
    public Integer insertEntryCreateId(T t) {
        Integer result = this.insertEntry(t);
        if (result > 0) {
            Integer id = (Integer) select(getNameSpace(DEFAULT_INSERT_LAST_SEQUENCE_KEY), Integer.class);
            if (id != null && id > 0) {
                try {
